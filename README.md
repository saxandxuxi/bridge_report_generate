...
- 名称对照表 `统计值/传感器名称对照/<桥名>.json` 里的 `测点映射`（应变/振动表 断面位置→测点N→编号）与 `表格映射`（位移/倾角/裂缝/温湿度/结构温度表）会在填表时自动生效，单元格命中来源记录在 `last_run.json → bridge.match_stats`。
...
| `name_dict` | 人工维护的名称→编号对照表（`统计值/传感器名称对照/<桥名>.json`），精确命中优先 |
